...
RSpec.describe TicketsMailer, type: :mailer do
  ...
  let(:buy_email) { described_class.send_ticket_notification(user, ticket, :buy) }
  let(:delete_email) { described_class.send_ticket_notification(user, ticket, :delete) }
  ...
  describe '.send_ticket_notification' do
    ...
    context 'when user delete ticket'
    it_behaves_like 'tickets_email' do
